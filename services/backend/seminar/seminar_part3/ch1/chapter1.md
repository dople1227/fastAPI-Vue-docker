...
- 사용자 모델과 이벤트 모델 두가지 모델을 사용한다.
- 사용자 라우팅은 사용자등록, 로그인 두가지 기능을 가진다.
- 이벤트 라우팅은 이벤트생성,단일이벤트조회,전체이벤트조회,이벤트변경,이벤트삭제 기능을 가진다.
...
- 각 사용자는 이벤트 필드를 가지며 여러개의 이벤트를 저장할 수 있다. (사용자:이벤트 -> 1:N 관계)
  
  ![Alt text](img/part3_ch1_image24.png)

<br/>

- 이벤트 모델(Event)을 models 폴더의 events.py에 정의한다.
  - Event 클래스 안에 Config 서브 클래스를 추가한다.  
    문서화할 때 샘플 데이터를 보여주기 위한 용도.
...
- 사용자 라우트 설계  
  - prefix /user
  - 등록(/signup), 로그인(/signin), 로그아웃(/signout)
