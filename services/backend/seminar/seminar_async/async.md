...
| Apache Kafka,RabbitMQ,...  메시지 전송 후 처리결과등을 알 필요가 없는 경우 | Socket.IO, Django Channels, ...                                                                                                    | Node.js, Express.js,...                                                                                                              |  |
| ![Alt text](img/async_image6.png)                                          | ![Alt text](img/async_image7.png)                                                                                                  | ![Alt text](img/async_image8.png)                                                                                                    |

  - FastAPI의 비동기 방식은 async/await를 기반으로 한 Coroutine을 방식을 사용
<br/>
<br/>
...
- FastAPI의 비동기는 기본적으로 병렬성(parallelism)이 아닌 병행성(concurrency)이다.
> 💡 병렬성(parallelism)과 병행성(concurrency)
>
> - 병렬성(parallelism)  
>   병렬성은 여러 작업이 실제로 동시에 실행되어 완료될 수 있는 것을 말한다
>   CPU코어가 여러개인 컴퓨터환경이라면 다중 CPU코어를 활용해 작업을 병렬적으로 처리할 수 있다.
>
> - 병행성(concurrency)  
>   여러 작업이 동시에 실행되는 것처럼 보이지만 실제로는 하나의 작업만 
>   실행되며, 실행되는 상태를 빠르게 변경하여 여러일을 마치 동시에 하듯 수행하는 것을 
>   말한다. 
> 
>   C++,Java등에서 여러 스레드를 실행한다는건 프로그램이 동시에 여러 CPU코어를 
>   사용해 각각 스레드가 동시에 실행된다는 것을 의미한다. Python도 멀티 스레드를  
>   지원하지만 GIL제약에 의해 실제로는 한번에 하나의 스레드만 실행하게 되고 다중 
>   CPU코어를 활용해 병렬적으로 실행되는 것이 제약된다.

| parallelism (병렬성)                          | concurrency (병행성,동시성)                   |
| --------------------------------------------- | --------------------------------------------- |
| ![Alt text](img/concurrency-parallelism2.png) | ![Alt text](img/concurrency-parallelism1.png) |

- 파이썬에선 기본적으로 병렬성이 아닌 병행성으로 비동기처리를 하기에 CPU바운드작업(계산집약적작업)에는  
  이점이 없지만 I/O작업(API호출,DB조회 등)에는 대기시간을 최소화하고 응답성을 향상시킬 수 있어 유용하다.

- 비동기 처리를 위해 사용하는 모듈 및 문법들   
  - asyncio  
    - FastAPI 내부적으로 asyncio를 사용해 비동기를 처리
    - 비동기 프로그래밍을 위한 핵심 모듈로 코루틴과 이벤트루프를 활용하여 비동기 작업을 처리한다.
      - 코루틴 (Coroutine)
        - 임시 중단 및 재개가 가능한 함수로, 비동기 작업의 일부를 표현
        - 'async' 키워드로 코루틴을 정의하고 'await' 키워드를 사용하여 작업완료를 기다리거나 다른 코루틴을 호출한다.
      - 이벤트 루프 (Event Loop)
        - 작성된 코루틴들을 실행하고 관리한다.
        - 비동기 작업의 상태를 추적하고, 작업이 완료되면 해당 작업을 재개하거나 결과를 처리한다.
  ---
  - httpx  
...
- 제너레이터 기반 코루틴과 네이티브 코루틴  
  - Python 3.5 이전 버전까지는 비동기를 구현하기 위해 'yield'키워드를  
  사용하는 제너레이터 기반의 코드를 주로 사용 했으나 3.5 버전부터 async/await 키워드가 도입되면서 현재는 네이티브 코루틴을 사용해 비동기를 구현하고 있다.

  | 구분                                               | 설명                                                                                                      |
  | -------------------------------------------------- | --------------------------------------------------------------------------------------------------------- |
  | 제너레이터 기반 코루틴 (Generator-based Coroutine) | 네이티브 코루틴 도입 전 비동기를 구현하기 위해 사용                                                       |
  |                                                    | 'yield'키워드를 사용하여 비동기 코드를 작성                                                               |
  |                                                    | 'yield'를 사용하여 값을 생성하고 'yield from'을 사용하여 다른 제너레이터나 코루틴에 대한 호출을 위임      |
  | 네이티브 코루틴 (native Coroutine)                 | Python 3.5버전부터 async/await 키워드가 도입되면서 사용                                                   |
  |                                                    | 'async' 와 'await' 키워드를 사용하여 비동기 코드를 작성                                                   |
  |                                                    | 'async def' 키워드를 사용하여 비동기 함수를 정의하고 'await'키워드를 사용하여 비동기 작업의 완료를 기다림 |
